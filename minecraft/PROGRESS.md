...
- `Creature.cs` (extends Enemy): flying 2-state AI (Idle / Grab). **Idle** — hovers in place, slowly Y-rotates toward the player, loops the `Idle` animation. Once line-of-sight to the player is clear (terrain-only raycast, layer 1), transitions to Grab. **Grab** — plays the `Grab` animation, lunges hard at the player (`LungeSpeed` / `GrabLungeAccel`). Damage is only dealt during a time-gated window within the animation (`GrabDamageStart`–`GrabDamageEnd`, default 0.5–1.3s), once per grab. Returns to Idle when the animation ends. Uses `TentacleCreature.glb` model with skeletal arms.
