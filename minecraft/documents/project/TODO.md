...
- [x] **Key collision fixed (2026-08-07):** `interactions.gd`'s raw keycode 69 (E) explode trigger is gone. It shared a key with `interact`, harmless in the hub but a real bug the moment E meant "use the warp point" on a destructible planet. `explode` (F) is the only trigger.
...
- [x] **`Sprites/planet icon.png` re-exported (2026-08-07)** — was a byte-identical copy of `planet destroyed icon.png` (a Krita export to the wrong filename), so every un-cleared planet drew the wreck. Fixed by re-export; the two files now differ (intact ringed planet vs. cracked one) and the bad copy survives as `planet icon.png~`. The code was always correct.
...
### Warp point structure (2026-08-07)

- [x] **The exit is a place, not a keypress.** Clearing the kill target now calls down the `"Warp Point"` structure: it falls from `fall_height` (70) above a spot `land_distance` (20) ahead of where the camera is looking, craters in, and looking at it and pressing `interact` (**E**) starts the same 10s charge. `Scripts/Handlers/WarpPoint.gd` on a `WarpPoint` node under `CubeLand.tscn`'s `Game`. `RunManager` still owns the countdown and `CompleteStage()`; the node is the only caller of `StartWarp()`.
- [x] **Interaction is block targeting, not a marker console.** Any block of the structure offers the prompt. A Marker1 like the ship's would leave a one-block hole in a solid pillar (markers are skipped at stamp time), which is why the ship's pattern isn't reused. `Global.HasSelectedBlock()`/`GetSelectedBlock()` wrap the DDA targeting `interactions.gd` already runs, tested against `GetStampBounds`' box — one raycast, and its 5-block reach doubles as the interaction radius.
- [x] Impact order is **explode then stamp** (the crater makes room for the pillar), radius derived from the structure's own footprint + `clear_margin`, clamped to `max_clear_radius` (8) because `explode` is O(r³) and this fires mid-combat. Stamped `clearAir = true`.
...
- [ ] The old **J keypress remains unverified** and is now only reachable on the failure path — synthetic input doesn't reach this environment's game window.
